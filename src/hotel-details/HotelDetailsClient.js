...
class HotelDetailsClient {
  constructor(hotelDetailsEndpointUrl, options) {
    ...
    self.poller = new Poller({
      ...
      pollLimit: 7,
      callApi: () => {
        let params = { currencyCode: self.currency.code, locale: self.locale, clientId: self.clientId, amountType: 'NIGHTLY' };
  ...
  searchHotelRates(search, mainSearchId) {
    ...
    if (mainSearchId !== undefined) {
      ...
      let params = { currencyCode: self.currency.code, locale: self.locale, amountType: 'NIGHTLY' };
